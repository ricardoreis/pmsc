...
$sql2 = "SELECT id FROM categorias WHERE nome = " . $DBConn->quote($nome);
$nome_existe = ($q = $DBConn->query($sql2)) && ($dados_categorias = $q->fetch());
...
$form_title = ($editando_registro ? 'Editar' : ($acao == 'add' ? 'Adicionar' : '')) . ' categorias';
$form_action = 'categorias/' . ($editando_registro ? $id . '/edit' : 'add');
