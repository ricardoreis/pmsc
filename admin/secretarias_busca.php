...
if (autorizado('secretarias')) {
//	Exclui registros selecionados	//
foreach ($delete as $v) {
	$DBConn->query("DELETE FROM secretarias WHERE id = " . $DBConn->quote($v));
}
//	=============================	//
	...
	foreach ($rows as $r) {
		...
		?><tr<?php echo (!(++$x % 2) ? ' class="alternate-row"' : '') ?>>
			...
			><td class="options-width">
				<a href="secretarias/<?php echo $r['id'] ?>/edit" class="icon-1 info-tooltip" title="Editar">Editar</a>
			</td>
		</tr><?php
	}
} else {
