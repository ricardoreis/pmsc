...
foreach ($delete as $v) {
	$DBConn->query("DELETE FROM noticias WHERE id_noticias = " . $DBConn->quote($v));
}
//	=============================	//

$sql = "SELECT id_noticias id,
			   titulo,
			   id_secretarias,
			   id_categorias,
			   texto,
...
$url_busca = 'noticias/busca/' . $qs_busca;
$url_order = $url_busca . ORDER_BY_TEMPLATE . $qs_paginacao . $qs_pagina;

$form_title = 'Notícias (módulo em construção)';
	...
	foreach ($rows as $r) {
		...
		?><tr<?php echo (!(++$x % 2) ? ' class="alternate-row"' : '') ?>>
			...
			<td class="options-width">
				<a href="noticias/<?php echo $r['id'] ?>/edit" class="icon-1 info-tooltip" title="Editar">Editar</a>
			</td>
		</tr><?php
	}
} else {
